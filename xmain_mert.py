...
def evaluate_syn_dataset(
    ...
    for model_eval in model_eval_pool:
        ...
        for it_eval in range(args.num_eval):
            ...
            print('    ', end='')
            print("************* Access Trigger:", accs_trigger)
            _, _, acc_test, acc_test_trigger= evaluate_synset_x(
                it_eval, net_eval, eval_syn_images, eval_syn_labels,
                testloader, testloader_trigger, args
            )
    ...
    for experiment in range(1, args.num_exp + 1):
        ...
        for iteration in range(args.Iteration + 1):
            ...
            for outer_iteration in range(args.outer_loop):
                print(args.outer_loop)
                print("Outer Iterations:",outer_iteration )
                for client in clients:
                ...
                if outer_iteration == args.outer_loop: # only record the final results
