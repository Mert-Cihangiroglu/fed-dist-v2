...
from networks import MLP, ConvNet, ConvNet_Original, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP
...
def evaluate_synset_x(it_eval, net, images_train, labels_train, testloader, testloader_trigger, args):
    net = net.to(args.device)
...
def select_neuron(net, layer, topk):
    ...
    ####
    #weights = torch.abs(logits_weights.cpu().detach())
    print(f"Weights shape before summing: {weights.shape}")
    sum_weights = torch.sum(weights, axis=1)  # This is where it breaks
    ####
